...
export async function handleModal(interaction) {
    ...
    switch (action) {
        ...
        default:
            console.warn(`[MODAL] Unknown action: ${action} (${interaction.customId})`);
            await interaction.reply({ content: '❓ Unknown modal action.', flags: MessageFlags.Ephemeral });
    }
}
...
// ═════════════════════════════════════════════════════════════════
//  GIVEAWAY CREATE MODAL SUBMIT
// ═════════════════════════════════════════════════════════════════

async function handleGiveawayCreate(interaction, targetId) {
    await interaction.deferReply({ flags: MessageFlags.Ephemeral });
    ...
    if (!config || channels.length === 0) {
        return interaction.editReply({ content: '❌ Giveaway system is not properly configured.' });
    }
    ...
    if (prize.length < 2 || prize.length > 100) {
        errors.push('• **Prize** must be 2–100 characters.');
    }

    const duration = parseInt(durationStr, 10);
    if (isNaN(duration) || duration < 1 || duration > 10080) {
        errors.push('• **Duration** must be 1–10080 minutes (up to 7 days).');
    }

    const winnerCount = parseInt(winnersStr, 10);
    if (isNaN(winnerCount) || winnerCount < 1 || winnerCount > 10) {
        errors.push('• **Winners** must be 1–10.');
    }
    ...
    try {
        ...
        // Post review in the same channel where command was used
        // Staff will see it and can approve/reject
        await sendStaffReview(interaction.guild, giveaway);

        const { formatDuration } = await import('../services/giveawayService.js');

        await interaction.editReply({
            content:
            `✅ **Giveaway submitted for approval!**\n\n` +
            `🎁 **Prize:** ${prize}\n` +
            `🏆 **Winners:** ${winnerCount}\n` +
            `⏱️ **Duration:** ${formatDuration(duration)}\n\n` +
            `Giveaway staff have been notified. You'll receive a DM when it's approved or rejected.\n` +
            `**Giveaway ID:** #${giveawayId}`,
        });

    } catch (err) {
        console.error('[GIVEAWAY] Create failed:', err);
        await interaction.editReply({ content: `❌ Failed to create giveaway: ${err.message}` });
    }
}

// ═════════════════════════════════════════════════════════════════
//  GIVEAWAY REJECT MODAL SUBMIT
// ═════════════════════════════════════════════════════════════════
...
async function handleGiveawayReject(interaction, giveawayIdStr) {
  ...
  if (giveaway.status !== 'pending') {
    return interaction.editReply({
      content: `⚠️ This giveaway has already been **${giveaway.status}** by another staff member.`,
    });
  }
  ...
  if (!acquireGiveawayLock(giveawayId)) {
    return interaction.editReply({
      content: '⏳ Another staff member is already processing this giveaway.',
    });
  }
  ...
  try {
    ...
    if (freshGiveaway.status !== 'pending') {
      return interaction.editReply({
        content: `⚠️ This giveaway was already **${freshGiveaway.status}** by another staff member.`,
      });
    }
    ...
    // Update status
    const { updateGiveawayStatus: updateStatus } = await import('../database/queries.js');
    updateStatus(giveaway.id, 'cancelled');

    // Fetch guild
    const guild = interaction.client.guilds.cache.get(giveaway.guild_id);

    // DM creator with rejection
    if (guild) {
      try {
        ...
        if (creator) {
          await creator.send({
            embeds: [
              new EmbedBuilder()
                .setTitle('❌ Your Giveaway Was Rejected')
                .setColor(COLORS.DANGER)
                .setDescription(
                  `Your giveaway for **${giveaway.prize}** in **${guild.name}** was rejected.\n\n` +
                  `📝 **Reason:** ${reason}\n` +
                  `❌ **Rejected by:** ${interaction.user.displayName}\n\n` +
                  `You can submit a new giveaway with \`/giveaway create\`.`,
                )
    ...
    await interaction.editReply({
      content: `✅ Giveaway **#${giveaway.id}** rejected. Creator has been notified.\n📝 **Reason:** ${reason}`,
    });

    console.log(`[GIVEAWAY] #${giveaway.id} rejected by ${interaction.user.username}: ${reason}`);

  } catch (err) {
    console.error('[GIVEAWAY] Reject failed:', err);
    await interaction.editReply({ content: `❌ Failed to reject: ${err.message}` });
  } finally {

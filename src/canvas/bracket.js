...
function statusColor(status) {
  switch (status) {
    case 'completed':   return C.DONE_BAR;
    case 'in_progress': return C.LIVE_BAR;
    case 'cancelled':   return C.CANCEL_BAR;
    default:            return C.PENDING_BAR;
  }
}
 ...
 * @param {Object<number,object[]>}   matchesByRound  round → match[]
 * @param {Map<string,object>}        participantMap  userId → { display_name, username }
 * @returns {Buffer}
 */
export function generateBracketImage(tournament, matchesByRound, participantMap) {
  ...
  const maxMatchesInRound = Math.max(...roundNumbers.map(r => matchesByRound[r].length));
  ...
  for (const r of roundNumbers) {
    ...
    const maxCols    = Math.max(1, Math.floor((canvasWidth - PAD * 2 + CARD_GAP_X) / (CARD_W + CARD_GAP_X)));
    const rowsNeeded = Math.ceil(matchCount / maxCols);
    canvasHeight += ROUND_HEADER_H + 8 + rowsNeeded * (CARD_H + CARD_GAP_Y) + ROUND_GAP;
  }
  canvasHeight += PAD;
  ...
  // ── Header ─────────────────────────────────────────────────
  const grad = ctx.createLinearGradient(PAD, y, canvasWidth - PAD, y + HEADER_H);
  ...
  ctx.fillText(truncate(ctx, tournament.name, canvasWidth - PAD * 2 - 60), PAD + 22, y + 34);

  ctx.fillStyle = 'rgba(255,255,255,0.7)';
  ctx.font = `13px ${FONT}`;
  const teamLabel   = tournament.team_size === 1 ? 'Solo' : 'Duo';
  const totalM      = Object.values(matchesByRound).flat().length;
  const completedM  = Object.values(matchesByRound).flat().filter(m => m.status === 'completed').length;
  ...
  for (const roundNum of roundNumbers) {
    ...
    const roundDone = matches.every(m => m.status === 'completed' || m.status === 'cancelled');
    ...
    // Round stats
    const roundCompleted = matches.filter(m => m.status === 'completed').length;
    ctx.fillStyle = C.TEXT_MUTED;
    ctx.font = `12px ${FONT}`;
    ctx.fillText(`${roundCompleted}/${matches.length} complete`, PAD + 120, y + 21);
...
function drawMatchCard(ctx, x, y, match, tournament, participantMap) {
  ...
  if (p1IsWinner) {
    ...
    ctx.fillRect(x + STATUS_BAR_W + 4, y + 2, CARD_W - STATUS_BAR_W - 8, CARD_H / 2 - 4);
  }

  ctx.fillStyle = p1IsLoser || isCancelled ? C.TEXT_MUTED : C.TEXT;
  ctx.font = p1IsWinner ? `bold 14px ${FONT}` : `14px ${FONT}`;
  ...
  const p1Score = isDone || status === 'in_progress' ? `${match.player1_score}` : '-';
  ...
  if (p2IsWinner) {
    ctx.fillStyle = C.WINNER_BG;
    ctx.fillRect(x + STATUS_BAR_W + 4, y + CARD_H / 2 + 2, CARD_W - STATUS_BAR_W - 8, CARD_H / 2 - 6);
  }

  ctx.fillStyle = p2IsLoser || isCancelled ? C.TEXT_MUTED : C.TEXT;
  ctx.font = p2IsWinner ? `bold 14px ${FONT}` : `14px ${FONT}`;
  ...
  ctx.font = `bold 14px ${FONT}`;
  const p2Score = isDone || status === 'in_progress' ? `${match.player2_score}` : '-';
...
function generateEmptyBracket(tournament) {
  ...
  // Header
  const grad = ctx.createLinearGradient(PAD, PAD, MIN_WIDTH - PAD, PAD + HEADER_H);
  grad.addColorStop(0, C.HEADER);
  grad.addColorStop(1, C.HEADER_END);
  ...
  ctx.fillText(truncate(ctx, tournament.name, MIN_WIDTH - 100), PAD + 22, PAD + 34);
  ...
  ctx.fillText('Bracket will appear once the tournament starts.', PAD + 16, PAD + HEADER_H + 42);

...
// Font family — falls back to sans-serif if Inter not registered
 ...
 * @returns {Buffer}
 */
export function generateLeaderboardImage(tournament, leaderboard, completedMatches, totalMatches) {
  const rows   = leaderboard.length || 1;
  const height = PAD + HEADER_H + 10 + INFO_H + 6 + COL_HEAD_H + 4 +
                 rows * (ROW_H + ROW_GAP) + 10 + FOOTER_H + PAD;
  ...
  ctx.fillText(truncate(ctx, tournament.name, CONTENT_W - 60), PAD + 24, y + 38);
  ...
  // ── Player rows ────────────────────────────────────────────
  const maxPts    = leaderboard.length > 0 ? Math.max(leaderboard[0].points, 1) : 1;
  ...
  for (let i = 0; i < leaderboard.length; i++) {
    ...
    ctx.font = isDQ ? `italic 15px ${FONT}` : `bold 15px ${FONT}`;
    ctx.fillText(truncate(ctx, p.display_name || p.username, COL.bar - COL.name - 16), COL.name, cy + 5);
  ...
  ctx.fillText(`Round Robin   •   Best of ${tournament.best_of}   •   ${teamLabel}   •   Max ${tournament.max_players} players`, PAD + 14, y + 23);

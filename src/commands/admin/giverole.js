...
export async function execute(interaction) {
  ...
  try {
    ...
    if (member.roles.cache.has(role.id)) return interaction.editReply({ content: `❌ **${user.displayName}** already has **${role.name}**.` });
    await member.roles.add(role, `Assigned by ${interaction.user.username}`);
    await interaction.editReply({ content: `✅ Assigned **${role.name}** to **${user.displayName}**.` });

    ...
    for (const match of allMatches) {
        ...
        if (match.thread_id) {
            if (match.status === 'in_progress') {
                // Match was active — show as completed with DQ note
                try {
                    await updateMatchThreadEmbed(guild, tournament, updatedMatch, true, `${opponentName} (opponent DQ'd)`);
                } catch {
                    // Thread may be archived or deleted
                }
            } else {
                // Match was pending but had a thread somehow — mark cancelled
                try {
                    await markThreadCancelled(guild, updatedMatch);
                } catch {
